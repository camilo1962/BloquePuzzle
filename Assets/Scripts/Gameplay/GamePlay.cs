...
public class GamePlay : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler, IBeginDragHandler
{
    ...
    void Start()
    {
        ...
        txtBestScore.text = "Record : " + BestScore.ToString();
        EnableSettingsMenu();
    ...
    /// <summary>
    /// Establece el temporizador para el progreso de la barra
    /// este método establecerá y restablecerá el temporizador al borrar la fila y la columna
    /// </summary>
    ...
    /// <summary>
    /// establece el progreso actual en barra.
    /// </summary>
    ...
    void OnBarCompelete()
    {
        StartCoroutine(LoadGameOver("¡La barra de tiempo había terminado!"));
    }

    /// <summary>
    /// Genera el evento de botón de deshacer presionado.
    /// </summary>
    ...
    /// <summary>
    /// Eleva el evento puntero hacia abajo.
    /// </summary>
    ...
    /// <summary>
    /// Provoca el evento de inicio de arrastre.
    /// </summary>
    /// <param name="eventData">Event data.</param>
    ...
    /// <summary>
    /// Levanta el evento puntero hacia arriba.
    /// </summary>
    /// <param name="eventData">Event data.</param>
    ...
    /// <summary>
    /// Coloca el objeto en el bloque dado.
    /// </summary>
    ...
    void BombCounter()
    {
        ...
        {
            StartCoroutine(LoadGameOver("¡El contador de bombas había terminado!"));
        }
    }
    ...
    void CheckForRestObjectPlacing()
    {
        ...
        if (objectToplace.Count > 0)
        {
            ...
            if (CanPlaceObject.FindIndex(o => o == true) == -1)
            {
                StartCoroutine(LoadGameOver("¡ Ya has PALMAO!, ¡No hay espacio para colocar mas bloques!"));
            }
        }
    }
    ...
    /// <summary>
    /// Comprueba si hay bloques vacíos.
    /// Comprobar que el objeto se coloca en este bloque o no
    /// </summary>
